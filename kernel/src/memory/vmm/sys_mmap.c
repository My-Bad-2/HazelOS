...
// void* sys_mmap(struct vm_space* space, void* addr, size_t length, int prot, int flags, int fd,
// long) {
//     if (!length || !space) {
//         return (void*)-EINVAL;
//     }
...
// int sys_mprotect(struct vm_space* space, void* addr, size_t length, int prot) {
//     if (!space || !addr || length == 0) {
//         return -EINVAL;
//     }
...
//             next_node ? rb_entry(next_node, struct vm_area, rb_node)->start : space->end_limit;
